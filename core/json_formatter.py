...
All fields mandatory. Sorted by suspicion_score descending.
Floating values rounded to 2 decimals.
fraud_rings now include optional "density_score" field.

Time Complexity: O(V log V) for sorting
Memory: O(V + R) where R = number of rings
"""
...
def format_output(
    ...
    Args:
        scores: {account_id: {score, patterns}}
        all_rings: All detected rings (may include density_score)
        total_accounts: Unique accounts in the dataset

    Returns:
        Dict matching the strict required JSON schema
    ...
    for ring in all_rings:
        ...
        # Optional density_score extension
    ...
    summary = {
        ...
        "processing_time_seconds": 0.0,  # filled by main.py after timing
    }
