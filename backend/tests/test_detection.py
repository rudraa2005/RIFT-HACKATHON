...
class TestSmurfingDetection:
    def test_fan_in_detected(self):
        rings, aggs, _, _ = detect_smurfing(_smurfing_fan_in_data())
...
class TestScoring:
    ...
    def test_cap_per_metric(self):
        ...
        # Cycle is 60 in config and cap is removed (or set to 100)
        base = s["ACC_001"]["score"]
        assert base >= 60  # Improved scoring for better detection
...
class TestJsonFormatter:
    ...
    def test_new_patterns_in_output(self):
        ...
        assert "rapid_forwarding" in patterns
        assert "low_retention" in patterns
...
class TestActivitySpike:
    def test_spike_detection(self):
        flagged, triggers = detect_activity_spikes(_activity_spike_data())
        assert isinstance(flagged, set)
    ...
    def test_no_spike_consistent(self):
        ...
        flagged, _ = detect_activity_spikes(pd.DataFrame(rows))
        assert "C001" not in flagged
...
class TestDiversityAnalysis:
    def test_burst_diversity(self):
        flagged, _ = detect_burst_diversity(_burst_diversity_data())
        assert isinstance(flagged, set)

    def test_merchant_not_flagged(self):
        flagged, _ = detect_burst_diversity(_merchant_data())
        assert "MERCHANT_001" not in flagged
...
class TestScenarioPurePassThrough:
    def test_correct_patterns(self):
        ...
        if mule:
            ...
            assert any(x in p for x in ["low_retention", "high_throughput", "rapid_forwarding"])
